...
const actionElement = document.getElementsByClassName('search__actions')[0];
...
function onSuggestionsResponse(data){
  ...
  if(suggestionsHTML){
    actionElement.classList.add('search__actions--autosuggest');
  }
  else {
    actionElement.classList.remove('search__actions--autosuggest');
  }
}
...
function getRandomString({length}){
  const characterChoices = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmonpqrstuvwxyz0123456789';
  const characters = [];
